...
void Analyser::process( SRSData *srs )
{
  ...
  for( int ih = 0; ih < nhits; ih++ )
  {
    ...
    SingleHit *CurrHit = new SingleHit;
    ...
    {
      SingleHit *PrevHit = new SingleHit;
      PrevHit = CurrentEvent[Gain].back();

      SingleHit *FHit = new SingleHit;
      FHit = CurrentEvent[Gain].front();

      int continuity = eventContinuity2( CurrHit, FHit, srs, ih, nhits );   // Properly define event
	...
	// std::vector< toCalibrate* > calEventTemp;
	...
	{ 
	  for( int j = 1; j < CurrentEvent[1].size() - 1; j++ )
	  {
	    ...
	    if( hg_charge != 0 )
	    {
	      ...
	      if( calMap.find( std::make_pair( vmm, channel ) ) == calMap.end() )
	      {
		MapGraph2 *Filler = new MapGraph2;
		TGraph *newGraph = new TGraph();

		Filler->vmm = vmm;
		Filler->channel = channel;
		Filler->graph = newGraph;
		
		calMap[std::make_pair( vmm, channel )].graph = newGraph;
		calVector.push_back( Filler );
	      }

	      double chargeRatio = hg_charge / CurrentEvent[1][j]->charge;
	      
	      if( hg_charge != 1023 &&
		  hg_charge >= graphChargeDownCutHG &&
		  chargeRatio >= graphChargeRatioCut &&
		  CurrentEvent[1].size() >= graphHitCut )
	      {
		auto it = calMap.find( std::make_pair( vmm, channel ) );
		TGraph* currGraph = it->second.graph;
		
		int np = currGraph->GetN();
		currGraph->SetPoint( np, CurrentEvent[1][j]->charge, hg_charge );
	      }
	...
	calEventHolder.push_back( Calibrator );
